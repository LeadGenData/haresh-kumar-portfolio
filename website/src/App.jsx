...
// ΓöÇΓöÇΓöÇ Canvas Particle Background ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Mouse Spotlight Ref Hook ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Project Card Spotlight Component ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇΓöÇ Interactive Terminal Console Component ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const InteractiveTerminal = () => {
  const [logs, setLogs] = useState([
    { type: 'info', text: 'system_initialize --target=haresh_portfolio' },
    { type: 'success', text: 'Γ£ö Data Ingestion Staging running (Snowflake integration active)' },
    { type: 'success', text: 'Γ£ö Power BI DAX semantic connector: ONLINE' }
  ]);
  ...
  const handleCommand = (cmd) => {
    ...
    setTimeout(() => {
      if (cleanCmd === 'help') {
        ...
        appendLog('warn', 'ΓÜí Launching Snowflake Bronze landing stream...');
        setTimeout(() => {
          appendLog('success', 'Γ£ö Bronze staging complete: variant rows stored');
          setTimeout(() => {
            appendLog('success', 'Γ£ö CDC streams validated in Silver schema (deduped)');
            setTimeout(() => {
              appendLog('success', 'Γ£ö Star dimension schema merged. Gold rebuild: SUCCESS');
            }, 500);
          }, 500);
        }, 400);
        ...
        setTimeout(() => {
          appendLog('success', 'Γ£ö Median Triage Wait time: 14.2 minutes (calculated)');
          appendLog('success', 'Γ£ö Patient satisfaction score: 92.4% success rating');
        }, 400);
      } else {
  ...
  return (
    <div className="terminal-container">
      ...
      <div style={{
        ...
        <button onClick={() => handleCommand('help')} className="btn btn-secondary" style={{ padding: '6px 12px', borderRadius: '6px', fontSize: '10px' }}>Γä╣∩╕Å Help</button>
        <button onClick={() => handleCommand('run pipeline')} className="btn btn-secondary" style={{ padding: '6px 12px', borderRadius: '6px', fontSize: '10px', color: 'var(--accent-cyan)', borderColor: 'rgba(34, 211, 238, 0.15)' }}>≡ƒÜÇ Run Medallion</button>
        <button onClick={() => handleCommand('dax')} className="btn btn-secondary" style={{ padding: '6px 12px', borderRadius: '6px', fontSize: '10px', color: 'var(--accent-gold)', borderColor: 'rgba(245, 158, 11, 0.15)' }}>≡ƒôê DAX Metrics</button>
      </div>
    </div>
  );
};
...
// ΓöÇΓöÇΓöÇ Career Chapter Switcher Timeline Data ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const CHAPTERS = [
  {
    id: 1,
    chapter: "Chapter 1",
    label: "Operations & Delivery (Innovays)",
    title: "Manager ΓÇö Data & Delivery",
    desc: "Led a core data operations and delivery team of 15+ members handling large enterprise accounts. Managed the processing and quality validation of 200KΓÇô300K+ monthly B2B records. Built Excel automation frameworks that reduced manual reporting cycles from 5 days down to 1 day.",
    stats: [
...
function App() {
  ...
  // Γ£à Saves to "Haresh Portfolio - Contact Inquiries" Google Sheet
  // Γ£à Sends notification email to hareshmkumar9@gmail.com
        ...
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: '64px' }} className="reveal">
            ...
            <p className="section-subtitle" style={{ margin: '0 auto' }}>
              I didn't start in a classroom. I started in the real world ΓÇö managing data at scale, leading teams, and delivering results directly to senior leadership.
            </p>
          </div>
